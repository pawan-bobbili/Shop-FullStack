...
const transporter = nodemailer.createTransport(sendgridtransport({
    auth : {
        api_key:'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'
    }
}))
...
exports.PostLogin = (req, res, next) => {
    ...
    User.findOne({email: req.body.email}).then(loggedin => {
...
exports.PostSignup = (req, res, next) => {
    ...
    if(! errors.isEmpty()){
        errors = errors.array();
        let i = 0, size = errors.length;
        let usernameError = "",emailError = "", keyError = "", key1Error =  ""
        while(i < size){
    ...
    bcryptjs.hash(req.body.key,9).then(pass => {
        const items = [];
        const user = new User({
